...
class DoubleQlearning(nn.Module):
    def __init__(
        ...
        self.state_values_1 = defaultdict(lambda: torch.zeros(num_actions))
        self.state_values_2 = defaultdict(lambda: torch.zeros(num_actions))
    
    def _forward(self, x: Tensor) -> Tensor:
        state_action_values = self.state_values_1[x] + self.state_values_2[x]
        policy = torch.where(state_action_values == torch.max(state_action_values), 1 - self.epsilon + (self.epsilon / self.num_actions), self.epsilon / self.num_actions)
        return policy
